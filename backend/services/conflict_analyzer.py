...
class ConflictAnalyzer:
    ...
    def _calculate_turn_conflict_score(
        ...
        if biases:
            bias_score = np.mean([
                bias_severity_map.get(b.get("severity", "medium"), 0.5)
                for b in biases
            ])
    ...
    def analyze_conversation(self, turns: List[Dict]) -> Dict:
        ...
        overall_conflict = np.mean(conflict_scores)
        passive_agg_index = np.mean(passive_agg_scores)
        ...
        return {
            ...
            "metrics": {
                "avg_aggression": np.mean(aggression_scores),
    ...
    def _predict_escalation(self, conflict_scores: List[float]) -> float:
        ...
        if len(conflict_scores) < 2:
            return conflict_scores[0] if conflict_scores else 0.0

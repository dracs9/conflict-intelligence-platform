...
from api.deps import get_simulation_service
...
async def simulate_response(
    ...
    opponent_model_db = (
        db.query(OpponentModel).filter(OpponentModel.session_id == session_id).first()
    )
    ...
    if not opponent_model_db:
        ...
        opponent_model = {
            ...
            "aggression_baseline": opponent_model_db.personality_profile.get(
                "aggression_baseline", 0.3
            ),
            "passive_aggression_baseline": opponent_model_db.personality_profile.get(
            ...
            "communication_style": opponent_model_db.personality_profile.get(
    ...
    simulation_result = simulation_service.simulate_response(
        request.user_draft, opponent_model, conversation_history
    )
...
async def get_opponent_profile(
    ...
    opponent_model = db.query(OpponentModel).filter(OpponentModel.session_id == session_id).first()

    if not opponent_model:
        # Build on-the-fly
        turns = (
            db.query(DialogueTurn)
            .filter(DialogueTurn.session_id == session_id, DialogueTurn.speaker == "opponent")
            .all()
        )
        ...
        profile = {
            ...
            "passive_aggression_baseline": opponent_model.personality_profile.get(
                "passive_aggression_baseline"
            ),
            "trigger_words": opponent_model.trigger_words,
        }

    return {"session_id": session_id, "opponent_profile": profile}

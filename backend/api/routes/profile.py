...
from database.models import ConflictSession, DialogueTurn, UserProfile
...
def _calculate_user_metrics(user_id: str, db: Session) -> dict:
    ...
    sessions = db.query(ConflictSession).filter(ConflictSession.user_id == user_id).all()
    ...
    for session in sessions:
        user_turns = (
            ...
            .filter(DialogueTurn.session_id == session.id, DialogueTurn.speaker == "user")
            .all()
        )
        ...
        # Session summary
        session_conflict = (
            float(np.mean([float(getattr(t, "conflict_score")) for t in user_turns]))
            if user_turns
            else 0.0
        )
    ...
    escalation_contribution = float(
        np.mean([float(getattr(t, "conflict_score")) for t in all_user_turns])
    )

    # Determine dominant style
    aggression_avg = float(np.mean([float(getattr(t, "aggression_score")) for t in all_user_turns]))
    passive_agg_avg = float(
        np.mean([float(getattr(t, "passive_aggression_score")) for t in all_user_turns])
    )
...
async def get_user_dashboard(user_id: str, db: Session = Depends(get_db)):
    ...
    if len(profile_data["conflict_history"]) > 1:
        recent_avg = np.mean([h["conflict_score"] for h in profile_data["conflict_history"][-3:]])
        older_avg = np.mean([h["conflict_score"] for h in profile_data["conflict_history"][:3]])
...
def _generate_insights(profile_data: dict) -> list:
    ...
    if profile_data["dominant_style"] == "constructive":
        insights.append(
            {"type": "positive", "message": "Great job! Your communication style is constructive."}
        )

    if profile_data["escalation_contribution"] < 0.3:
        insights.append(
            {"type": "positive", "message": "You're good at keeping conflicts from escalating."}
        )

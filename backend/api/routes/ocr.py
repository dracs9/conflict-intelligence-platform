...
async def upload_screenshot(
    ...
    # Create session
    session = ConflictSession(user_id=user_id, session_name=f"Screenshot Upload {file.filename}")
    db.add(session)
    db.commit()
    db.refresh(session)
    ...
    for turn_data in extracted_turns:
        # Analyze turn
        analysis = conflict_analyzer.analyze_turn(turn_data["text"], turn_data["speaker"])
...
async def extract_text(file: UploadFile = File(...)):
    ...
    return {"extracted_text": text, "character_count": len(text)}

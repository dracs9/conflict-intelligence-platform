...
from database.models import ConflictSession, DialogueTurn, ConflictAnalysis
import main
...
async def analyze_session(session_id: int, db: Session = Depends(get_db)):
    ...
    # Perform analysis
    analysis_result = main.conflict_analyzer.analyze_conversation(turn_data)
...
async def get_latest_analysis(session_id: int, db: Session = Depends(get_db)):
    ...
    analysis = db.query(ConflictAnalysis).filter(
        ConflictAnalysis.session_id == session_id
    ).order_by(ConflictAnalysis.created_at.desc()).first()
